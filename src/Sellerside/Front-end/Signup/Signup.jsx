...
// CSS import removed as we're using Tailwind directly
...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${BaseUrl}/customer/register/`, {
        name: form.name,
        username: form.email,
        phoneno: form.phone,
  ...
  return (
    <div className="flex min-h-screen font-sans bg-gradient-to-r from-white to-blue-50">
      {/* Left Section - Hidden on md breakpoint and below */}
      <div className="w-1/2 bg-blue-600 text-white flex flex-col justify-center px-20 py-12 relative overflow-hidden md:block hidden" id="left-section"> 
      ...
      {/* Right Section */}
      <div className="md:w-1/2 w-full flex items-center justify-center px-16 text-brown" id="right-section">
          {/* Heading - Hidden by default, visible on mobile */}
          <div className="hidden md:hidden absolute top-[1%] md:top-[1%] sm:top-[2%] text-lg md:text-lg sm:text-sm xs:text-xs font-black bg-[#f0e5d0] p-[10px_20px] rounded-[20px] md:block sm:flex sm:justify-center sm:items-center" id="right-section-heading">
            <h1>REGISTER AND SELL YOUR PRODUCTS HERE !</h1>
          </div>
        <div className="max-w-md w-full md:w-full sm:w-[90vw] bg-white p-10 rounded-xl shadow-xl md:static sm:absolute sm:top-[80px] md:shadow-xl sm:shadow-[10px_5px_15px_rgba(0,0,0,0.1),-10px_-5px_15px_rgba(0,0,0,0.1)]" id="right-section-form">
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="mb-4">
              <input
                ...
                value={form.email}
                onChange={handleChange}
                className="w-full px-4 py-3 transition-all duration-300 focus:w-[120%]  border border-gray-300 rounded-lg  focus:ring-blue-400"
              />
            ...
            <button
              ...
              className={`w-full text-white py-2.5 rounded-lg font-semibold transition duration-200 ${
                register ? "bg-green-950 hover:bg-black-900 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
              }`}
            >
